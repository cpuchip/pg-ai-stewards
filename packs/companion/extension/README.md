...
- `stewards_companion.control` — `requires = 'pg_ai_stewards'`,
  `default_version = '0.2.0'`, `relocatable = false` (the pack creates its
...
- `stewards_companion--0.2.0.sql` — a fresh install lands here directly.
...
-- or pin the first release and upgrade:
...
eight stages to green: virgin 0.1.0 install → 0.1.0 smoke (+ steward-tools
absent) → `ALTER … UPDATE 0.2.0` smoke → fresh 0.2.0 → dump/restore survival →
