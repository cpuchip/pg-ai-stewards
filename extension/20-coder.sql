...
-- (B5/19 rule); the code-pr critic defaults to qwen3.7-max and dev to kimi-k2.6
-- (both name-only strings; unrowed models default usable).
-- =====================================================================
...
INSERT INTO stewards.stage_models (pipeline_family, stage_name, default_model, notes) VALUES
    ...
    ('code-pr',     'plan_review', 'qwen3.7-max', 'Plan critic (cv11): reviews the plan vs acceptance criteria before build; PLAN: approved -> implement, PLAN: revise -> loop back to plan (capped).'),
    ...
    ('code-pr',     'review',      'qwen3.7-max', 'Plan-conformance critic (cv6): a DIFFERENT strong model than the implementer. REVIEW: passes -> pr, REVIEW: revise -> loop back to implement (capped, then awaiting_review).'),
