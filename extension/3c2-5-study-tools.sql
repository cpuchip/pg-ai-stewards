...
--   - Phase 2.3 (refresh_doc_similarity edges)
...
--   2. stewards.doc_search(query, kinds[], limit) — FTS over
--      studies.body_tsv with multi-kind filter.
--   3. stewards.doc_get(slug, include_body, line_offset, line_count,
...
-- whose tool_pattern allow-list matches `doc_*`) emits these tools
...
-- ---------------------------------------------------------------------
-- Underlying SQL function: doc_search
--
...
'Phase 3c.2.5: FTS over stewards.docs.body_tsv. Multi-kind filter via array (empty = all). Ordered by ts_rank.';

-- ---------------------------------------------------------------------
-- Underlying SQL function: doc_get
--
...
(
    'doc_similar',
    'Return docs semantically similar to the given slug, using precomputed pgvector cosine similarity edges (Phase 2.3). No on-the-fly embedding; cheap. Each result has a score (0..1, higher = more similar) and direction (outgoing | incoming | mutual). Use after doc_search to expand a topic''s neighborhood.',
    '{
        "type": "object",
        "required": ["slug"],
        "properties": {
...
-- yet. Substrate-internal tools like doc_search don't match
-- those patterns, so the deny-* fallback wins and compose_tools
-- emits nothing useful.
--
-- We blanket-allow `doc_*` across all non-watchman agents. The
...
--
-- glob_match: `doc_*` is length 7, beats `*: deny` (length 1)
