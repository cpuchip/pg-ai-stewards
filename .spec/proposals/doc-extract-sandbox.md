...
**Status:** ✅ **BUILT + SHIPPED 2026-06-24** (autonomous /goal run on Opus 4.8). All phases P3a–P3f + P4 landed + proven; chain 00→49; pushed. RATIFIED in council 2026-06-24 (`dominion_in_council` satisfied). See `docs/rich-documents.md` for the architecture + port guide; journal `.spec/journal/2026-06-24-rich-docs-p3-doc-extract.md`.
**Author:** agent + Michael, 2026-06-24.
**What shipped:** `internal/docextract` (pure-Go converter core + safety oracle) · `cmd/doc-extract` (in-sandbox converter) · `cmd/doc-extract-mcp` (`doc_extract` + `doc_import_corpus`, bridge-side) · `extension/doc-extract.Dockerfile` (Go + poppler + ClamAV) · `docker-compose.doc-extract.yaml` (freshclam + clamav-db) · `49-doc-extract.sql` · ChatPanel doc-attach + project lens picker. Proven: go-test oracle (zip-slip/bomb/structural) · trust-floor smoke (EICAR quarantined by ClamAV, macro flagged, `--network=none` no-egress) · virgin-smoke 00→49 (OK 36–39) · live tool path (PDF → text + auto-rendered pixels; archive → searchable corpus) · live deployment (bridge catalog has both tools; UI lens picker renders). Also fixed a latent topo-sort revert of the multimodal guard (47 `requires`).
