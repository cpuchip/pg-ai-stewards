...
| **D · OTel export** | everything-is-a-row + Stewdio | emit OTLP spans per turn/tool-call from rows that already exist | S · high |
...
4. **OTel export (miss D, steal #7).** Smallest effort, real adoption value — emit OTLP spans from rows that already exist. → *act-safe, oracled.*
