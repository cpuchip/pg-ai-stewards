...
3. **Abort-materialization → NEW TABLE `work_item_abort_conditions`** (corrected 2026-07-05 — an earlier typo read Michael's "leaning" as "leaving/reuse"). Aborts are first-class, inspectable rows — `(work_item_id, kind, params jsonb, source_move, armed, tripped_at, tripped_reason)` — joining the ~9 existing per-work-item side tables (gate_decisions, verify_results, needs_attention, …). Rationale: the spiral oracle is a stateless *function*, not a per-item condition store, so "reuse" would mean a jsonb blob interpreted in the bgworker — the "config buried in a field" anti-pattern the substrate exists to replace. A core table (owned by a chain file, versioned — cannot be lost like the operator seeds) makes the war-game's foresight queryable, reportable, and demoable ("armed aborts → watch one trip → escalate"). **Predicate is STRUCTURED, never model-authored SQL**: `kind` from a fixed evaluator vocabulary (`error_matches`, `tool_unavailable`, `repeat_failure`, `budget_fraction`, …) + `params` — same discipline as edge-vocabulary/route_on. **Forks still → `route_on`, assumptions still → `ask_up`** (those are their right homes); only *aborts* get the new table. Clean hybrid: new surface where the novel per-item thing lives, reuse where a good home exists.
