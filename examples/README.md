...
| [`case-file-digester.sql`](case-file-digester.sql) | Drop a folder of paperwork in, get an inspectable **case file** out: deterministic section split + typed fact floor (v29 normalize), citation-sanity check (`citation_check` — does the cited section say what the letter claims?), server-rendered timeline/denial-map/evidence-checklist, and a draft letter as the one model-authored section. Ends at the case file; **no send capability exists**. Demo pack + runnable spine: [`case-file-demo/`](case-file-demo/). | `psql "$STEWARDS_DSN" -f examples/case-file-digester.sql`, then `case-file-demo/demo.sh` |
