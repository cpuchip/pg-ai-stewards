...
func DownloadVideoFile(cfg *Config, rawURL string, force bool, maxHeight int, cookieOverride string) (string, *VideoMetadata, error) {
	...
	// Last resort `/18` = the 360p progressive mp4 (a single combined stream, no
	// merge): when the capped bestvideo+bestaudio adaptive formats 403 (YouTube
	// throttling the DASH URLs), the progressive itag often still serves. Slides
	// stay legible at 360p, so a degraded frame beats no frame.
	format := fmt.Sprintf("bestvideo[height<=%d]+bestaudio/best[height<=%d]/best/18", maxHeight, maxHeight)
