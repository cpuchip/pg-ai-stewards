...
func (s *MCPServer) handleToolsList(enc *json.Encoder, req *MCPRequest) {
	tools := []map[string]any{
			...
			"inputSchema": map[string]any{
				...
				"properties": map[string]any{
					"channel": map[string]any{
						"type":        "string",
						"description": "Filter by channel slug (e.g., 'two-minute-papers')",
					},
		...
		{
			"name":        "yt_playlist",
			"description": "List the videos on a YouTube PLAYLIST or channel URL WITHOUT downloading them (via yt-dlp --flat-playlist). Returns each video's id, title, and url in playlist order. This is the discovery step the download-first tools lack: use it to see what a playlist contains, then yt_download the ones you want. Requires yt-dlp.",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
					"url": map[string]any{
						"type":        "string",
						"description": "YouTube playlist or channel URL (e.g., 'https://www.youtube.com/playlist?list=...').",
					},
					"limit": map[string]any{
						"type":        "integer",
					...
					"cookies": map[string]any{
						"type":        "string",
						"description": "Path to a Netscape cookies.txt for authentication, if the playlist requires sign-in.",
					},
				},
				"required": []string{"url"},
			},
...
func (s *MCPServer) handleToolsCall(enc *json.Encoder, req *MCPRequest) {
	...
	case "yt_playlist":
		s.handleYtPlaylist(enc, req, params.Arguments)
	default:
...
func (s *MCPServer) handleYtPlaylist(enc *json.Encoder, req *MCPRequest, args json.RawMessage) {
	var input struct {
		URL     string `json:"url"`
		Limit   int    `json:"limit"`
		Cookies string `json:"cookies"`
	...
	if input.URL == "" {
		s.sendToolError(enc, req.ID, "url is required (a YouTube playlist or channel URL)")
		return
	}

	entries, err := ListPlaylist(s.cfg, input.URL, input.Limit, input.Cookies)
	if err != nil {
		s.sendToolError(enc, req.ID, fmt.Sprintf("Playlist listing failed: %v", err))
		return
	}

	if len(entries) == 0 {
		s.sendToolResult(enc, req.ID, "No videos found on that URL (is it a playlist or channel?).")
		return
	}

	var response string
	response += fmt.Sprintf("Playlist has %d videos (flat list, no transcripts downloaded). Pick one by id, then yt_download its url:\n\n", len(entries))
	for i, e := range entries {
		response += fmt.Sprintf("%d. id=%s | %s | %s\n", i+1, e.ID, e.Title, e.URL)
	}
	s.sendToolResult(enc, req.ID, response)
}
