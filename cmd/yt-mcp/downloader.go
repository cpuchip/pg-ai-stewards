...
// e.g., "Two Minute Papers" → "two-minute-papers"
...
// PlaylistEntry is one video on a playlist (from --flat-playlist, no download).
...
// ListPlaylist enumerates the videos on a playlist (or channel) URL WITHOUT
// downloading any of them, via `yt-dlp --flat-playlist --dump-json`. Returns
// each entry's id/title/url in playlist order. This is the discovery half the
// download-first tools (yt_download/yt_get) lack: it lets a caller see what
// exists on a playlist and pick which videos to fetch.
func ListPlaylist(cfg *Config, playlistURL string, limit int, cookieOverride string) ([]PlaylistEntry, error) {
	if limit <= 0 {
		limit = 50
	}
	args := []string{"--flat-playlist", "--dump-json", "--playlist-end", strconv.Itoa(limit)}
	args = append(args, cookieArgs(cfg, cookieOverride)...)
	...
	out, err := cmd.Output()
	if err != nil {
		return nil, fmt.Errorf("yt-dlp --flat-playlist failed (is this a playlist/channel URL?): %w", err)
	}

	// --dump-json emits one JSON object per line (NDJSON).
	var entries []PlaylistEntry
	sc := bufio.NewScanner(bytes.NewReader(out))
	sc.Buffer(make([]byte, 0, 1024*1024), 16*1024*1024) // playlist lines can be large
	for sc.Scan() {
