...
// stewards.doc_citations() expansion. Each cited_uri that maps to a
...
func (d *Deps) graphStudiesCitationsHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Substrate's doc_citations returns cited_uri (often a path); we extract
	// the basename without .md as a candidate slug.
	for _, n := range resp.Nodes {
		erows, err := d.Pool.Query(ctx,
			`SELECT cited_uri, citation_count FROM stewards.doc_citations($1)`,
			n.ID,
		)
		if err != nil {
			continue
