...
// lifting (doc_search for FTS, doc_similar for pgvector
// cosine, doc_citations for relational-graph derived edges, doc_get
// for the row body).
...
func listCountQuery(kind string) string {
	if kind == "" {
		return `SELECT count(*) FROM stewards.docs`
	}
	return `SELECT count(*) FROM stewards.docs WHERE kind = $1`
}
...
func (d *Deps) studiesGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Citations — substrate's doc_citations(p_slug) returns
	// (doc_slug, cited_uri, cited_kind, anchor_text, citation_count).
	...
	// Similar studies — doc_similar(p_slug, p_limit) returns
	...
	rows2, err := d.Pool.Query(ctx,
		`SELECT slug, title, score
		   FROM stewards.doc_similar($1, 10)`,
		slug,
	)
	if err == nil {
		defer rows2.Close()
...
func (d *Deps) studiesSearchHandler(w http.ResponseWriter, r *http.Request) {
	...
	switch mode {
	case "fts", "combined":
		// doc_search(p_query, p_kinds=ARRAY[]::text[], p_limit)
